...
class ChaptersDownloader(object):
    ...
    async def run(self) -> None:
        ...
        urls = [(chapter, all_chapters_urls[chapter])
                for chapter in self.chapters]

        self.chapters_output_path.mkdir(parents=True, exist_ok=True)
        async with asyncio.TaskGroup() as tg:
            for url in urls:
                tg.create_task(self._process_chapter(*url))
    ...
    async def _get_chapters_urls(self) -> Mapping[int, str]:
        ...
        return {
            _get_chapter_number(a.attrs["href"].split("/")[-1]):
            f'{BASE_URL}{a.attrs["href"]}'
    ...
    async def _process_chapter(self, chapter: int, url: str) -> None:
        ...
        async with asyncio.TaskGroup() as tg:
            im_tasks = [
                tg.create_task(self._download_image(im_url))
                for im_url in im_urls
            ]

        # 3. Concatenate all images in a single PDF
        ims = [task.result() for task in im_tasks]
...
def _get_chapter_number(s: str) -> int | None:
